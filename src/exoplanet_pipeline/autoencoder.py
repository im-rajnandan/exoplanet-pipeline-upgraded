"""
================================================================
 exoplanet_pipeline/autoencoder.py
...
try:
    ...
    HAS_TORCH = False
    DEVICE = "cpu"
    # Create a dummy class so nn.Module references don't crash at load time
    class nn:
        class Module:
            pass
...
class AutoencoderTrainer:
    ...
    def __init__(self, input_length=201, latent_dim=32, lr=1e-3, 
                 batch_size=64, epochs=100, patience=10, model_dir="data/models"):
        if not HAS_TORCH:
            raise ImportError("PyTorch is required for AutoencoderTrainer.")
    ...
    def _prepare_data(self, global_views, sde_values=None, sde_threshold=6.0):
        ...
        if len(train_data) == 0:
            train_data = global_views
            print("[AUTOENCODER] WARNING: No boring curves found. Training on all curves instead.")
    ...
    def train(self, global_views, sde_values=None, sde_threshold=6.0):
        ...
        for epoch in range(self.epochs):
            ...
            if val_loss < best_val_loss:
                ...
                torch.save(self.model.state_dict(), 
                           os.path.join(self.model_dir, 'autoencoder_best.pth'))
            else:
                patience_counter += 1
        ...
        self.model.load_state_dict(
            torch.load(os.path.join(self.model_dir, 'autoencoder_best.pth'),
        ...
        # 99th percentile anomaly threshold to avoid over-filtering planets
        self.threshold = np.percentile(train_errors, 99)
        print(f"\n  Anomaly threshold (99th pctile): {self.threshold:.6f}")
        ...
        with open(os.path.join(self.model_dir, 'autoencoder_config.json'), 'w') as f:
    ...
    def save(self, path=None):
        if path is None:
            path = os.path.join(self.model_dir, 'autoencoder_best.pth')
    ...
    def load(self, path=None):
        ...
        config_path = os.path.join(self.model_dir, 'autoencoder_config.json')
